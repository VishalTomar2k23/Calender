...
//Variables Declaration
  ...
  {
    if(event.endDate>=this.currentDate.toISOString().split('T')[0])
    {
...
openEventForm(Day: number) {
    ...
    if (selectedDate >= this.currentDate || this.countupdate==1) {
 ...
 getEventClass(event: any): { class: string, width: string } {
  ...
  if (dayDiff > 0) {
    ...
    if (endDate > breakDate) {
      ...
      width = `${(dayDiff + 1) * 210 + (dayDiff-1) * 10}px`;
      eventClass = isPastEvent ? 'event-grey' : 'event-green';
    }
  }
